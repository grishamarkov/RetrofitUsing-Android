...
public class PlaybackActivity extends Activity {
    ...
    private LeanbackPlaybackState mPlaybackState = LeanbackPlaybackState.IDLE; // default state
    private MediaSession mSession;
    private String mCurrentVideoPath;
    private long mDuration = -1;

    private Medium mSelectedMovie = null;
    ...
    private void updateMetadata(final Medium movie) {
        final MediaMetadata.Builder metadataBuilder = new MediaMetadata.Builder();

        String title = movie.getTitle().replace("_", " -");

        metadataBuilder.putString(MediaMetadata.METADATA_KEY_MEDIA_ID, movie.getTitle());
...
//        metadataBuilder.putString(MediaMetadata.METADATA_KEY_ARTIST, movie.getStudio());
        ...
        Glide.with(this)
            .load(Uri.parse(movie.getPicture()))
            .asBitmap()
    ...
    private void loadViews() {
        ...
        String json = getIntent().getStringExtra(MovieDetailsActivity.MOVIE);
        Type type = new TypeToken<Medium>() {}.getType();
        mSelectedMovie = new Gson().fromJson(json, type);

        try {
            setVideoPath(mSelectedMovie.getSourceUrl());
...
//        return true;
    ...
    private class MediaSessionCallback extends MediaSession.Callback {
        ...
        public void onPlayFromMediaId(String mediaId, Bundle extras) {
            Medium movie = getMovieById(mediaId);

            try {
                setVideoPath(movie.getSourceUrl());
            }
            catch (Exception e) {
                e.printStackTrace();
            }
    ...
    private Medium getMovieById(String mediaId) {
