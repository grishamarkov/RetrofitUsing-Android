...
public class PlaybackOverlayFragment extends android.support.v17.leanback.app.PlaybackOverlayFragment {
    ...
    private ArrayList<Medium> mItems = new ArrayList<Medium>();
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        mItems = new ArrayList<Medium>();
        String json = getActivity().getIntent().getStringExtra(MovieDetailsActivity.MOVIE);
        Type type = new TypeToken<Medium>() {}.getType();
        mSelectedMovie = new Gson().fromJson(json, type);

        String json1 = getActivity().getIntent().getStringExtra(MovieDetailsActivity.MOVIELIST);
        Type type1 = new TypeToken<Media>(){}.getType();
        mSelectedMedia = new Gson().fromJson(json1, type1);

        List<Medium> mediumList = mSelectedMedia.getMedia();
        for (int j = 0; j < mediumList.size(); j++) {
            mItems.add(mediumList.get(j));
            if (mSelectedMovie.getTitle().contentEquals(mediumList.get(j).getTitle())) {
                mCurrentItem = j;
            }
        }
    ...
    private int getDuration() {
        try {
            mDuration = Utils.getDuration(mItems.get(mCurrentItem).getSourceUrl());
    ...
    private void updateMovieView(String title, String studio, String cardImageUrl, long duration) {
        if (mPlaybackControlsRow.getItem() != null) {
            Medium item = (Medium) mPlaybackControlsRow.getItem();
            item.setTitle(title);
...
//        mMediaController.getTransportControls().playFromMediaId(mItems.get(mCurrentItem).getId(), bundle);
    ...
    static class DescriptionPresenter extends AbstractDetailsDescriptionPresenter {
        ...
        protected void onBindDescription(ViewHolder viewHolder, Object item) {
            viewHolder.getTitle().setText(((Medium) item).getTitle());
            viewHolder.getSubtitle().setText(((Medium) item).getDescription());
        }
    }
