...
import com.n2me.androidtv.activities.PlaybackActivity;
...
import com.n2me.androidtv.presenter.DetailsDescriptionPresenter;
...
public class MovieDetailsFragment extends android.support.v17.leanback.app.DetailsFragment {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        String json = (String) getActivity().getIntent().getStringExtra(MovieDetailsActivity.MOVIE);
        Type type = new TypeToken<Medium>() {}.getType();
        mSelectedMovie = new Gson().fromJson(json, type);

        String jsonMedia = (String) getActivity().getIntent().getStringExtra(MovieDetailsActivity.MOVIELIST);
        Type type1 = new TypeToken<Media>() {}.getType();
        mSelectedMedia = new Gson().fromJson(jsonMedia, type1);
        ...
        if (mSelectedMovie != null || checkGlobalSearchIntent()) {
            ...
            setupSocialRow();
            setupSocialRowPresenter();
            updateBackground(mSelectedMovie.getPicture());
...
//                });
    ...
    private void setupDetailsOverviewRow() {
        Log.d(TAG, "doInBackground: " + mSelectedMovie.getTitle());
        final DetailsOverviewRow row = new DetailsOverviewRow(mSelectedMovie);
        row.setImageDrawable(getResources().getDrawable(R.drawable.default_background));
        int width = Utils.dpToPx(DETAIL_THUMB_WIDTH, getActivity().getApplicationContext());
        int height = Utils.dpToPx(DETAIL_THUMB_HEIGHT, getActivity().getApplicationContext());
        Glide.with(getActivity())
                .load(mSelectedMovie.getPicture())
                .centerCrop()
        ...
        row.addAction(new Action(ACTION_WATCH_TRAILER, "", "Play"));
        ...
        detailsPresenter.setOnActionClickedListener(new OnActionClickedListener() {
            ...
            public void onActionClicked(Action action) {
                if (action.getId() == ACTION_WATCH_TRAILER) {
                    ...
                    // add medium to watched preference
                    N2MEPreference.addMedium(mSelectedMovie);

                    // play medium
                    Intent intent = new Intent(getActivity(), PlaybackActivity.class);
                    String json = new Gson().toJson(mSelectedMovie);
                    intent.putExtra(MovieDetailsActivity.MOVIE, json);

                    String json1 = new Gson().toJson(mSelectedMedia);
                    intent.putExtra(MovieDetailsActivity.MOVIELIST, json1);
...
//        mAdapter.add(new ListRow(header, listRowAdapter));
    ...
    private void setupSocialRow() {

    }

    private void setupSocialRowPresenter() {
